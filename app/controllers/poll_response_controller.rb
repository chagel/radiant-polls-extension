  ...
  def create
    ...
    if !session[:submitted_polls].include?(poll.id) && (!current_poll || current_poll.id == poll.id)
      ...
      if params[:response_id]
        for id in params[:response_id]
          poll_response = Option.find(id)
          poll_response.submit_response
        end
      end
      poll.submit_response
    end
    ...
    redirect_to params[:return_url].blank? ? @page.url : params[:return_url]
  end
end
